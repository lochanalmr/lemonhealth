...
def BMIcalcclass():
    ...
    print("\n\nWe recommend using metric units for calculations, but using imperial units is also available as an option")
...
def MultiMeasure():
    ...
    if unit1=='m':
        ...
        print("Note:- Gender/Regional data is only used for classification purposes")
        gender1=input("Insert M if Male, or F if Female: ")
        age1 = basicData()
        print("\nThe program needs to check whether the user's region is asia, as asian health classifications are different from other regions")
        region1=input("Insert A if Asia, or O if in other region: ")
        print("\nAll required data is now collected.")
        print("Your inputs are being processed ...")
        bmi=weight1m/(height1m**2)
        bai=hc1m/(height1m**1.5-18)
        whr=hc1m/wc1m
